...
in
{
  ...
  config = lib.mkIf cfg.enable {
    services.bazarr = {
      ...
      openFirewall = false;
      # Run as Maxwell:users — same as services.sonarr / services.radarr
      # on this host. The TV/Movies dirs are owned by Maxwell:users with
    ...
    systemd.services.bazarr-chown = {
      ...
      script = ''
        if [ -d /var/lib/bazarr ]; then
          chown -R Maxwell:users /var/lib/bazarr
        fi
      '';
    };
