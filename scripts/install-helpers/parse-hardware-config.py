...
class HardwareConfigParser:
    ...
    def parse_boot_config(self) -> Dict:
        ...
        initrd_modules = re.search(
            r"boot\.initrd\.availableKernelModules\s*=\s*\[([^\]]+)\]", self.config_text
        )
        if initrd_modules:
        ...
        kernel_modules = re.search(
            r"boot\.kernelModules\s*=\s*\[([^\]]+)\]", self.config_text
        )
        if kernel_modules:
    ...
    def parse_hardware_modules(self) -> List[str]:
        ...
        imports_match = re.search(
            r"imports\s*=\s*\[(.*?)\];", self.config_text, re.MULTILINE | re.DOTALL
        )
    ...
    def detect_partition_scheme(self) -> Dict:
        ...
        scheme = {"type": "unknown", "partitions": [], "disk_size_gb": 0}
