...
if nix search nixpkgs "vscode-extensions.${EXTENSION_ID}" &>/dev/null; then
  ...
  echo "   cd /home/Maxwell/.config/nixos"
  ...
  if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    VSCODE_FILE="/home/Maxwell/.config/nixos/home/development/vscode.nix"
    ...
    if grep -q "vscode-extensions.${EXTENSION_ID}" "$VSCODE_FILE"; then
      ...
      echo "Now run: cd /home/Maxwell/.config/nixos && sudo nixos-rebuild switch --flake ."
    fi
  fi
