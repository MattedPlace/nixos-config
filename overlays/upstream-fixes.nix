final: prev: {
  ...
  python314 = prev.python314.override (old: {
    packageOverrides = prev.lib.composeExtensions (old.packageOverrides or (_: _: { })) (
      _pyfinal: pyprev: {
        ...
        optuna = pyprev.optuna.overridePythonAttrs (_o: {
          doCheck = false;
        });
        cheetah3 = pyprev.cheetah3.overridePythonAttrs (_o: {
  ...
  ollama-rocm = prev.ollama-rocm.overrideAttrs (_old: {
    doCheck = false;
  });
  ollama-cuda = prev.ollama-cuda.overrideAttrs (_old: {
    doCheck = false;
  });
