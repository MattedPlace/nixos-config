...
let
  username = "Maxwell";
  hostname = "k3sagent1";
  masterIP = "192.168.1.202"; # Updated to match the master's IP from the other config
  k3sToken = "7j2hK6sVjkzN5sE8sF+pQyXlJd3w8bX0y5ZvX7K9KAo=";
  mac = "02:00:00:00:00:02"; # Different MAC from master
  ip = "192.168.1.203/24"; # Different IP from master
in
{
  ...
  microvm = {
    ...
    # Network interface configuration - using tap interface like master
    interfaces = [
      {
        type = "tap";
        id = hostname;
    ...
    volumes = [
      {
        mountPoint = "/var";
        image = "var-agent1.img";
        size = 2048;
      }
      {
        mountPoint = "/var/lib/rancher/k3s";
        image = "k3s-agent1-data.img";
